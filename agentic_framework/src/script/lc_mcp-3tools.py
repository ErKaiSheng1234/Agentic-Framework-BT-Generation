...
def sys_prompt():
    instruction_xml = (
        ...
        "STEP 3 — Generate navigation snippet: Call get_navtopose_node_xml() with the goal names "
        "and coordinates from the user's task.\n"
        "  - If the task gives coordinates (x, y, yaw), convert them to semicolon format "
        "(e.g., '-3.34;-3.87;2.82') and pass as the value.\n"
        "  - If coordinates are NOT provided, pass an empty string '' as the value.\n"
        "  - Example: get_navtopose_node_xml({'cooling_tower_1': '-3.34;-3.87;2.82'})\n"
        "  - The tool returns raw XML. For the task with coordinates provided, it gives BOTH <SetBlackboard> (to store coords) AND "
        "<NavToPose> (to navigate). For the task without coordinates provided, it gives <NavToPose> (to navigate) only. Use the returned XML directly.\n\n"

        "STEP 4 — Compose and output: Wrap the XML from step 3 inside the control structure from the task "
        "(e.g., wrap in <Sequence>, add <RetryUntilSuccessful> if the task says retry). "
        "Then wrap everything in the root structure below. Output the final XML and STOP.\n\n"
        ...
        "CRITICAL STOP RULES:\n"
        "- After step 4, output your answer and STOP. Do NOT call any more tools.\n"
        "- Never call the same tool twice with the same parameters.\n"
        "- Once you have the XML assembled, you are DONE. Output it immediately.\n"
        "- Output ONLY valid raw XML. No markdown fences (no ```xml), no explanations, no extra text.\n"
        "- Limit yourself to exactly 4 tool calls maximum per task. If you get valid output from step 3, "
        "proceed directly to step 4 and finish.\n"
    )
...
mcp_tool_config = {
        "behavior_tree_node_info" : {
            "transport": "stdio",
            "command": "python3",
            "args": ["/workspaces/agentic_framework/src/mcp_tools/btnode_info_server_3tools.py"]
        },
    }
...
async def bt_generation(
    ...
    for idx, task in enumerate(data, start=1):
        ...
        # # 2. Flush/save every 100 records
        # if idx % 100 == 0:
        #     print(f"Saving batch of {len(results_list)} results to {output_file_path}...")
...
if __name__ == "__main__":
    ...
    async def bt_gen_wrapper(model, temp, reasoning):
        ...
        async with model_locks[model]:
            ...
            await bt_generation(
                agent=agent,
                output_file_path=f"/workspaces/result/result-3tools/{model}_{temp}_{think}.json"
            )
