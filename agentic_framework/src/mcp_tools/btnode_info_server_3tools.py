...
# SCHEMA_PATH = resources.files('agentic_framework').joinpath('src/mcp_tools/info/bt_node_schema_no_example.json')
SCHEMA_PATH = '/workspaces/agentic_framework/src/mcp_tools/info/bt_node_schema_no_example.json'
...
async def get_nodes_info(node_list: list[str]) -> dict:
    ...
    Call this SECOND, after get_nodes_name, to learn the exact syntax of the nodes you need.
    
    Args:
        node_list: A list of node type names (e.g., ["SetBlackboard", "NavToPose", "Sequence", "RetryUntilSuccessful"]) 
...
async def get_navtopose_node_xml(targets: Dict[str, str]) -> Dict[str, str]:
    """
    Generates the complete Behavior Tree XML snippet for navigation to one or more goal poses.

    IMPORTANT: This tool returns BOTH a <SetBlackboard> node (to store the coordinate) AND a
    <NavToPose> node (to navigate) when coordinates are provided. When coordinates are empty,
    it returns only the <NavToPose> node.

    Call this THIRD, after get_nodes_info, with the goal names and coordinates from the user's task.

    Args:
        targets: A mapping of goal names to their coordinate string values.
                 - Key (str): The goal name (e.g., "kitchen", "charging_station").
                 - Value (str): The coordinate string (e.g., "1.2;3.4;0.0"). Use semicolons (;), NOT commas.
                                If coordinates are unknown or managed elsewhere, pass an empty string "".

    Returns:
        Dict[str, str]: A dictionary mapping each goal name to its raw XML string (no markdown fences).
                        Wrap the returned XML inside your control structure (Sequence, RetryUntilSuccessful, etc.)
                        and the root Behavior Tree tags.
    ...
    for goal, coord in targets.items():
        ...
        # RETURN RAW XML — no markdown code fences, so the LLM can use it directly
